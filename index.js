const stripAnsi = require('strip-ansi').default; // fow wemovin \x1b[...] stuf
	...
	for (const def of roleMessages){
		...
		if (!msg){
			...
			for (const emo of Object.keys(def.mapping)){
				try {await msg.react(emo);} catch (e){ console.error('react failed', emo, e);}
			}
		}
...
client.on("messageCreate", message => {
	...
	const trimd = (message.content || '').trim();
	if (/(?:https?:\/\/)?(?:www\.)?(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}(?:\/[^\s]*)?/gi.test(trimd)) return;
	...
	const noPings = message.content.replace(/<@[!&]?\d+>|<#!?\d+>/g, ''); // y is this here only for the 67 resp?
	if (six.test(noPings) && seven.test(noPings)){
...
client.once("clientReady", () => {
	...
	const restart = new SlashCommandBuilder()
		.setName("restart")
		.setDescription("Restarts the minecraft servurr (if noone playing)");
	client.application.commands.create(restart, serverId);

	const stop = new SlashCommandBuilder()
		.setName("stop")
		.setDescription("Stops the minecraft servurr (if noone playing)");
	client.application.commands.create(stop, serverId);
	
	const mcStatus = new SlashCommandBuilder()
		.setName("status")
		.setDescription("Shows status of the mc servurr");
...
client.on(Events.InteractionCreate, interaction => {
	...
	if(interaction.commandName === "restart"){
		...
		exec("/run/current-system/sw/bin/mcrcon -H localhost -P 25575 -p mcservurrpasswd list | grep -oP 'There are \\K\\d+'", (error, stdout, stderr) => {
			if (error){
				...
				tts("cannot restart servurr, " + stdout.trim() + " people online", interaction, "reply");
			}
		});
	}
